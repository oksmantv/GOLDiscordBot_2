...
class ScheduleCommands(commands.Cog):
    ...
    @app_commands.command(
        ...
        description="Cancel a scheduled event (marks it as EVENT CANCELLED)",
    )
    @app_commands.describe(event="Select the event to cancel")
    async def cancelscheduledevent_command(
        self,
        interaction: discord.Interaction,
        event: str,
    ):
        """Handle the /cancelscheduledevent command."""
        guild = interaction.guild
        if not guild:
            await interaction.response.send_message(
                "❌ This command can only be used in a server.", ephemeral=True
        ...
        try:
            ...
            if success:
                ...
                if config:
                    ...
                    if channel:
                        try:
                            ...
                            await interaction.followup.send(
                                f"Event cancelled, but failed to update schedule message: {e}",
                                ephemeral=True,
                            )
                            return
                ...
                event_date_str = selected_event.date.strftime("%A, %d %B %Y")
                await interaction.followup.send(
                    f"✅ **{selected_event.type}** on **{event_date_str}** has been marked as **EVENT CANCELLED**.",
                )
            else:
                await interaction.followup.send(
                    "❌ Failed to cancel event. Please try again later.",
                    ephemeral=True,
                )
        except Exception as e:
            await interaction.followup.send(
    ...
    @cancelscheduledevent_command.autocomplete("event")
    async def cancelscheduledevent_event_autocomplete(
        self,
        interaction: discord.Interaction,
        current: str,
    ) -> list[app_commands.Choice[str]]:
        """Autocomplete for cancelscheduledevent event selection."""
        try:
            available_events = await date_filter_service.get_available_events(
                search=current
            )
